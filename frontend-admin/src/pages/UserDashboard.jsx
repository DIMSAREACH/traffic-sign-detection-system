...
function formatViolationType(vtype = "") {
  return String(vtype).replace(/_/g, " ").replace(/\b\w/g, (m) => m.toUpperCase());
}
...
function MiniCard({ icon, title, value, sub, color, bg }) {
  return (
    <div className="col-12 col-lg-6">
      <div className="card border-0 rounded-4 h-100"
        style={{ background: "var(--bs-body-bg)", boxShadow: "0 2px 12px rgba(124,58,237,.06)" }}>
        <div className="card-body p-3 d-flex align-items-center gap-3">
          <div className="rounded-3 d-flex align-items-center justify-content-center flex-shrink-0"
            style={{ width: 46, height: 46, background: bg, color }}>
            <i className={`bi ${icon}`} style={{ fontSize: "1.35rem" }} />
          </div>
          <div className="flex-fill" style={{ minWidth: 0 }}>
            <div className="fw-bold" style={{ fontSize: "1rem" }}>{title}</div>
            {sub && <div className="text-secondary" style={{ fontSize: ".88rem" }}>{sub}</div>}
          </div>
          <div className="fw-bold" style={{ fontSize: "1.3rem", color: "var(--bs-body-color)" }}>
            {value ?? "—"}
          </div>
        </div>
      </div>
...
export default function UserDashboard() {
  ...
  const recent = Array.isArray(stats?.recent_violations) ? stats.recent_violations : [];
      ...
      <div className="flex-fill overflow-auto no-scrollbar" style={{ minHeight: 0 }}>
        ...
        <div className="row g-3 mb-3">
          <MiniCard
            ...
            value={stats ? money(stats.total_paid_amount) : "—"}
            sub="Sum of paid fines linked to your violations"
            color="#16a34a"
          ...
          <MiniCard
            ...
            value={latest?.location ? "Seen" : "—"}
            sub={latest?.location ? `${latest.location}` : "No violations recorded yet"}
            color={PU}
            bg={PA(".10")}
          />
        </div>
            ...
            <div className="row g-2">
              <div className="col-6 col-md-3">
                <QAction icon="bi-exclamation-triangle-fill" label={t("nav.myViolations")}
                  to="/dashboard/violations" color="#ef4444" bg="rgba(239,68,68,.06)" />
              </div>
              <div className="col-6 col-md-3">
                <QAction icon="bi-receipt-cutoff" label={t("nav.myFines")}
                  to="/dashboard/fines" color="#f59e0b" bg="rgba(245,158,11,.06)" />
              </div>
              <div className="col-6 col-md-3">
                <QAction icon="bi-credit-card-2-front" label={t("nav.myPayments")}
                  to="/dashboard/payments" color="#22c55e" bg="rgba(34,197,94,.06)" />
              </div>
              <div className="col-6 col-md-3">
                <QAction icon="bi-car-front-fill" label={t("nav.vehicles")}
                  to="/dashboard/vehicles" color={PU} bg={PA(".06")} />
              </div>
            <div className="col-6 col-md-3">
              <QAction icon="bi-person-circle" label={t("nav.profile")}
                to="/dashboard/profile" color={PU} bg={PA(.06)} />
            </div>
            </div>
          </div>
        </div>
              ...
              <div className="list-group list-group-flush">
                {stats.recent_violations.map((v) => (
                  <div key={v.id} className="list-group-item border-0 px-0 d-flex justify-content-between align-items-center"
                    ...
                    <div>
                      <span className="fw-semibold" style={{ color: "var(--bs-body-color)" }}>{formatViolationType(v.violation_type)}</span>

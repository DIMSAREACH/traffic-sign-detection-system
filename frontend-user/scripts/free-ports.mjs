...
// User dev server uses 5173; only free this port so we don't kill the admin app.
const ports = [5173];
...
function dbg(message, data, hypothesisId = "A", runId = "pre-fix") {
  try {
    // Prefer file-based logs for Node scripts (more reliable than HTTP in some shells).
    fs.appendFileSync(
      "debug-b6ba61.log",
      `${JSON.stringify({
        sessionId: "b6ba61",
        runId,
        hypothesisId,
        location: "frontend-user/scripts/free-ports.mjs",
        message,
        data,
        timestamp: Date.now(),
      })}\n`,

...
import { hasStoredAuthCredentials, primeAccessTokenIfStale } from "../services/api.js";
...
export default function Users() {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          const noResponse = Boolean(e?.request) && !e?.response;
          const transient =
            st === 429 ||
            (Number(st) >= 500 && Number(st) < 600) ||
            (noResponse && ["ECONNABORTED", "ERR_NETWORK"].includes(String(e?.code || "")));
  ...
  useEffect(() => {
    if (authLoading && hasStoredAuthCredentials()) return;
    load("", "all", "all", 1);
    loadStats();
  }, [authLoading, me?.id]); // eslint-disable-line — refetch when session / profile is ready
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg =
        ...
        || (Array.isArray(e?.response?.data?.non_field_errors) && e.response.data.non_field_errors[0])
        || (typeof e?.response?.data === "string" ? e.response.data : null)
        || e?.message;
      showToast(msg || t("usr.deleteFailed"), "error");
    } finally {

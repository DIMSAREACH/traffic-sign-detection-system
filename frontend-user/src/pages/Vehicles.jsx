...
export default function Vehicles() {
  ...
  useEffect(() => {
    ...
    // Admin-only: used to assign vehicle owners when creating/editing vehicles.
    api.get("/auth/users/", { params: { page_size: 1000 } })
      .then((r) => {
        const items = r.data.results ?? r.data ?? [];
        // only keep users that have driver_profile
        const driversOnly = items.filter((u) => u?.role === "driver" || u?.driver_profile);
        // normalize to expected shape used by driverDisplayName()
        setDrivers(driversOnly.map((u) => ({ id: u.driver_profile?.id ?? u.id, user: u })));
      })
      .catch(() => {});
  }, []);
